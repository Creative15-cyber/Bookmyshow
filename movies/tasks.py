...
def send_ticket_email_task(self, order_id):
    """
    Generates the PDF ticket and emails it to the order's user.

    Runs entirely in a Celery worker process, separate from the web
    request that triggered payment confirmation - the HTTP response for
    that request has already been sent back to the browser well before
    this task even starts running, let alone finishes.

    autoretry_for=(Exception,) means ANY failure here (SMTP timeout, DNS
    issue, transient network blip, etc.) is automatically retried with
    exponential backoff, up to 5 attempts, without any special handling
    in this function - Celery re-invokes the whole task from scratch on
    each retry.
    """
    ...
    try:
        ...
        logger.error("send_ticket_email_task: Order %s does not exist, not retrying.", order_id)
        return "order_not_found"

    if order.status != "paid":
        logger.warning(
            "send_ticket_email_task: Order %s is not paid (status=%s), skipping email.",
            order_id, order.status,
        )
        return "order_not_paid"

    if not order.user.email:
        logger.warning("send_ticket_email_task: Order %s user has no email on file.", order_id)
